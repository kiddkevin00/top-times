...
class UnconnectedTimeManagement extends Component {
  ...
  handleInputChange(field, eventOrValue) {
    this.props.dispatchSetFormField(field, eventOrValue && eventOrValue.target ? eventOrValue.target.value : eventOrValue);
  }
  ...
  render() {
    ...
    const timeZones = data.map(t => t)
      .filter(timeZone => {
        let willBeIncluded = true;

        if (formSearch) {
          const searchAgainst = `${timeZone.displayName} ${timeZone.cityInTimeZone} ${timeZone.customCityName || ''} ${timeZone.timeZone} ${timeZone.diffToBrowserTime}`.toLowerCase();
        ...
        if (willBeIncluded && (formShowNearbyTimeZonesOnly || formShowCurrentTimeZoneOnly)) {
          ...
          if (formShowNearbyTimeZonesOnly && 1 === 2) { // TODO: Use `diffToBrowserTime`
            willBeIncluded = true;
          }

          if (formShowCurrentTimeZoneOnly && 1 === 3) { // TODO: Use `diffToBrowserTime`
            willBeIncluded = true;
          }
        }
            ...
            <div className="panes">
              ...
              <section className="pane">
                <List
                  ...
                  renderItem={item => (
                    <List.Item
                      actions={[<span key="list-edit" className="link">Edit</span>, <span key="list-delete" className="link">Delete</span>]}
                    >
                      <Skeleton loading={false} avatar={true} title={false} active={true}>
                        <List.Item.Meta
                          avatar={<ReactCountryFlag countryCode={item.countryCode} svg={true} className="flag-icon" />}
                          title={`${item.displayName} (${item.customCityName || item.cityInTimeZone})`}
                          description={`${format(new Date(), 'hh:mm aaaa')} (${item.diffToBrowserTime})`}
                        />
