...
class UnconnectedUserManagement extends Component {
  ...
  deleteUser(id) {
    this.props.dispatchRemoveUser(id);
  }
  ...
  render() {
    ...
    const usersDataSource = users
      .filter(user => {
        ...
        if (formSearch) {
          const searchAgainst = `${user.fullName} ${user.timeZone} ${user.role} ${user.email} ${format(parseISO(user.dateCreated), 'M/d/yyyy')}`.toLowerCase();
        ...
        if (willBeIncluded && (formShowManagersOnly || formShowAdminsOnly || formShowNewsletterSubscribersOnly)) {
          willBeIncluded = false;
      ...
      .sort((first, second) => {
        if (formSortBy) {
          ...
          if (formSortBy === usersSortBy.timeZone) {
            return first.timeZone.localeCompare(second.timeZone);
          }
        }
                ...
                <List
                  ...
                  renderItem={item => {
                    ...
                    return (
                      <List.Item
                        actions={[
                          <Link key="list-manage-time-zone" to={`/manage-timezones/${item._id}`}>Manage Timezones</Link>,
                          <Link key="list-edit" to={`/account-info/${item._id}`}>Edit</Link>,
                          <span key="list-delete" className="link" onClick={this.deleteUser.bind(this, item._id)}>Delete</span>,
                        ]}
                      >
                        <Skeleton loading={false} avatar={true} title={false} active={true}>
                          <List.Item.Meta
                            avatar={<Avatar icon={icon} shape="square" size="large" className="role-icon" />}
