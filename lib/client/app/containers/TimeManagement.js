  ...
  render() {
    ...
    const timeZonesDataSource = timeZones
      .map(timeZone => {
        ...
        const currentTimeInTimeZoneConvertedToCurrentTimeZone = moment(currentTimeInTimeZone.format('YYYY-MM-DDTHH:mm:ss.SSSSSSSSS'));
        ...
        const diffToBrowserTime = Math.round(currentTimeInTimeZoneConvertedToCurrentTimeZone.diff(moment(), 'hours', true));
        ...
        return {
          ...
          currentTime,
          diffToBrowserTimeDisplay: `${dateDiff}, ${diffToBrowserTime >= 0 ? '+' : ''}${diffToBrowserTime} HRS`,
          diffToBrowserTime,
        };
      })
      .filter(timeZone => {
      ...
      .sort((first, second) => {
        if (formSortBy) {
          ...
          if (formSortBy === timeZonesSortBy.city) {
            return (first.customCityName || first.cityInTimeZone).toLowerCase().localeCompare((second.customCityName || second.cityInTimeZone).toLowerCase());
          }
        ...
        <main>
          <div className="white-board">
            <div className="main-header">
              ...
              <Button type="button" onClick={() => history.push(`/manage-timezone${isManagingUser ? `?userId=${match.params.userId}` : ''}`)}>
                Add
              </Button>
            </div>
            <div className="panes">
              ...
              <section className="pane">
                <List
                  ...
                  renderItem={item => (
                    <List.Item
                      actions={[
                        <Link key="list-edit" to={`/manage-timezone?timeZoneId=${item._id}${isManagingUser ? `&userId=${match.params.userId}` : ''}`}>
                          Edit
                        </Link>,
                        <span key="list-delete" className="link" onClick={() => this.props.dispatchRemoveTimeZone(item._id, match.params.userId)}>
                          Delete
                        </span>,
                      ]}
                    >
                      <Skeleton loading={false} avatar={true} title={false} active={true}>
                        <List.Item.Meta
                          ...
                          title={`${item.displayName} (${item.customCityName ||
                            item.cityInTimeZone})`}
                          description={<span style={{ fontSize: '17px' }}>{`${item.currentTime} (${item.diffToBrowserTimeDisplay})`}</span>}
                        />
